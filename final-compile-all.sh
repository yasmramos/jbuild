...
# JBuild Complete Multi-Module Compilation Script
# Script final para compilar todo el proyecto JBuild usando el sistema nativo
...
compile_module() {
    ...
    if [ -s "$TEMP_BUILD/${module_name}_files.txt" ]; then
        local file_count=$(wc -l < "$TEMP_BUILD/${module_name}_files.txt")
        ...
        # Compilar
        if [ -n "$classpath" ]; then
            javac -d "$output_dir" -cp "$classpath" @${TEMP_BUILD}/${module_name}_files.txt 2>/dev/null
        else
            javac -d "$output_dir" @${TEMP_BUILD}/${module_name}_files.txt 2>/dev/null
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            log "🔍 Mostrando algunos errores:"
            
            # Re-ejecutar con errores visibles
            if [ -n "$classpath" ]; then
                javac -d "$output_dir" -cp "$classpath" @${TEMP_BUILD}/${module_name}_files.txt 2>&1 | head -5
            else
                javac -d "$output_dir" @${TEMP_BUILD}/${module_name}_files.txt 2>&1 | head -5
            fi
            return 1
        fi
    else
